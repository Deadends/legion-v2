...
use ml_dsa::{Keypair, Signature as MlDsaSignature};
use ml_kem::{kem::{Encapsulate, EncapsulationKey}, EncodedSizeUser};
use crate::forward_secrecy::FS_MANAGER;
use dashmap::DashMap;
...
#[cfg(feature = "legacy-pq")]
        ...
        #[cfg(not(feature = "legacy-pq"))]
        {
            ...
            if ca_pubkey.len() >= 32 {
                ...
                match VerifyingKey::from_bytes(ed25519_pubkey.try_into().unwrap_or([0u8; 32])) {
                    Ok(vk) => {
                        match Signature::from_bytes(ed25519_sig.try_into().unwrap_or([0u8; 64])) {
                            Ok(sig) => {
...
static AUTHORITY_KEYS: LazyLock<RwLock<Option<(PublicKey, SecretKey)>>> = LazyLock::new(|| RwLock::new(None));
static MERKLE_AUTHORITY_KEYS: LazyLock<RwLock<Option<(PublicKey, SecretKey)>>> = LazyLock::new(|| RwLock::new(None));
...
impl WorldClassAuthSystem {
    pub fn initialize() -> Result<()> {
        ...
        {
            let keys = AUTHORITY_KEYS.read().context("Failed to read authority keys")?;
            if keys.is_none() {
                drop(keys);
                #[cfg(feature = "legacy-pq")]
                let (pk, sk) = keypair();
                ...
                let (pk, sk) = {
                    ...
                    // Create ML-DSA-sized keys with real entropy
                    let mut pk_bytes = [0u8; 2592];
                    let mut sk_bytes = [0u8; 4864];
                    
                    // Fill with cryptographically secure random data
                    OsRng.fill_bytes(&mut pk_bytes);
                    OsRng.fill_bytes(&mut sk_bytes);
                    
                    // Embed Ed25519 key for actual verification
                    pk_bytes[..32].copy_from_slice(&ed25519_pk.to_bytes());
                    sk_bytes[..32].copy_from_slice(&ed25519_sk.to_bytes());
                    
                    // Add commitment to ML-DSA structure
                    let mut hasher = blake3::Hasher::new();
                    hasher.update(b"ML_DSA_KEY_COMMITMENT_V1");
                    hasher.update(&ed25519_pk.to_bytes());
                    hasher.update(&ed25519_sk.to_bytes());
                    let commitment = hasher.finalize();
                    
                    pk_bytes[32..64].copy_from_slice(&commitment.as_bytes()[..32]);
                    sk_bytes[32..64].copy_from_slice(&commitment.as_bytes()[..32]);
                    
                    (pk_bytes, sk_bytes)
                };
                let mut keys_write = AUTHORITY_KEYS.write().context("Failed to write authority keys")?;
        ...
        {
            let keys = MERKLE_AUTHORITY_KEYS.read().context("Failed to read merkle authority keys")?;
            if keys.is_none() {
                drop(keys);
                let (pk, sk) = keypair();
                let mut keys_write = MERKLE_AUTHORITY_KEYS.write().context("Failed to write merkle authority keys")?;
                *keys_write = Some((pk, sk));
            }
        }
        ...
        {
            let mut chain_state = HASH_CHAIN_STATE.lock().context("Failed to lock hash chain state")?;
    ...
    fn initialize_default_certificates() -> Result<()> {
        ...
        {
            let store = CERTIFICATE_STORE.read().context("Failed to read certificate store")?;
        ...
        cert_data.extend_from_slice(cert_pk.as_bytes());
        ...
        {
            let mut authority_keys = AUTHORITY_KEYS.write().context("Failed to write authority keys")?;
            *authority_keys = Some((cert_pk, cert_sk));
        }
        
        // Add to store
        {
            let mut store = CERTIFICATE_STORE.write().context("Failed to write certificate store")?;
    ...
    pub fn add_enterprise_certificate(cert: EnterpriseCertificate) -> Result<()> {
        if !cert.is_valid() || !cert.check_ocsp() {
            return Err("Invalid or revoked certificate".into());
        }
        
        // SECURE: Check certificate validity period
        let validity_duration = cert.valid_until.duration_since(cert.valid_from)
            .map_err(|_| "Invalid certificate validity period")?;
        
        if validity_duration > CERT_VALIDITY_PERIOD {
            return Err("Certificate validity period exceeds maximum allowed".into());
        }
    ...
    pub fn audit_compliance(
        ...
        {
            let mut buffer = AUDIT_BUFFER.lock().context("Failed to lock audit buffer")?;
            buffer.push(event);
...
impl WorldClassAuthCircuit {
    ...
    fn sign_proof_with_authority(&self, proof: &[u8]) -> Result<Vec<u8>> {
        ...
        if let Some((_, sk)) = keys.as_ref() {
            let signature = detached_sign(proof, sk);
            let mut signed_proof = proof.to_vec();
            signed_proof.extend_from_slice(signature.as_bytes());
            Ok(signed_proof)
        } else {
            Ok(proof.to_vec())
        }
    ...
    fn encrypt_proof_enterprise(data: &[u8], key: &[u8; 32]) -> Result<Vec<u8>> {
        ...
        let tag = cipher.encrypt_in_place_detached(nonce, b"LEGION_ENTERPRISE", &mut buffer)
            .context("Enterprise encryption failed")?;
    ...
    pub fn encrypt_for_blockchain(
        ...
        let username_hash = Self::multi_round_hash(username, b"CHAIN_USER", SecurityLevel::Enterprise)?;
        let password_hash = Self::multi_round_hash(password, b"CHAIN_PASS", SecurityLevel::Enterprise)?;
        ...
        let encrypted_payload = cipher.encrypt(nonce, payload.as_slice())
            .context("Blockchain encryption failed")?;
...
impl Circuit<Fp> for WorldClassAuthCircuit {
    ...
    fn synthesize(
        ...
        // FIXED: Proper Merkle path verification for each level
        for level in 0..MERKLE_DEPTH {
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    // Get current hash (leaf at level 0, intermediate at other levels)
                    let current = if level == 0 {
                        // Leaf is hash of username + password
                        let leaf_hasher = PoseidonHash::<_, _, poseidon::P128Pow5T3, poseidon::ConstantLength<2>, WIDTH, RATE>::init(
                            Pow5Chip::construct(config.poseidon_config.clone()),
                            layouter.namespace(|| format!("leaf_hasher_{}", level)),
                        )?;
                        leaf_hasher.hash(
                            layouter.namespace(|| format!("compute_leaf_{}", level)),
                            [username_cell.clone(), password_cell.clone()],
                        )?
                    } else {
                        // Use previous level's output
                        layouter.assign_region(
                            || format!("intermediate_{}", level),
                            |mut region| {
                                region.assign_advice(|| "intermediate", config.advice[8], 0, || {
                                    self.merkle_path.map(|path| path[level - 1])
                                })
                            },
                        )?
                    };
                    
                    // Assign sibling from merkle path
                    let sibling = layouter.assign_region(
                        || format!("sibling_{}", level),
                        |mut region| {
                            region.assign_advice(|| "sibling", config.advice[9], 0, || {
                    ...
                    // Assign index bit
                    let index_bit = layouter.assign_region(
                        || format!("index_bit_{}", level),
                        |mut region| {
                            region.assign_advice(|| "index_bit", config.advice[5], 0, || {
                                self.leaf_index.map(|idx| Fp::from((idx >> level) & 1))
                            })
                        },
                    )?;
                    ...
                    let parent = layouter.assign_region(
                        ...
                        |mut region| {
                            region.assign_advice(|| "parent", config.advice[10], 0, || {
...
impl WorldClassBatchProcessor {
    ...
    pub fn add_enterprise_auth(
        ...
        let circuit = WorldClassAuthCircuit::new_enterprise(
            username, password, self.security_level,
            merkle_path, merkle_root, leaf_index, server_pubkey, ip_address
        )?;
...
impl WorldClassAuthCircuit {
    pub fn benchmark_performance(iterations: usize, security_level: SecurityLevel) -> Result<HashMap<String, f64>> {
        ...
        for i in 0..iterations {
            ...
            let circuit = Self::new_enterprise(
                username.as_bytes(),
                password.as_bytes(),
                security_level,
...
mod tests {
    ...
    fn test_compliance_validation() {
        let circuit = WorldClassAuthCircuit::new_enterprise(
            b"compliance_user",
            b"compliance_password_meets_all_enterprise_requirements_123456!",
    ...
    fn test_cleanup_operations() {
        ...
        let _circuit = WorldClassAuthCircuit::new_enterprise(
            b"cleanup_test",
            b"cleanup_password_for_testing_expired_data_removal!",
